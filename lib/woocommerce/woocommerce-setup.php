 ...
 * GC Twenty.
 *
 * This file adds the required WooCommerce setup functions to the GC Twenty Theme.
 *
 * @package GC Twenty
 * @author  StudioPress
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'gc_twenty_woocommerce_breakpoint' );
/**
 ...
 */
function gc_twenty_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'gc_twenty_default_products_per_page' );
/**
 ...
 */
function gc_twenty_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'gc_twenty_woocommerce_pagination' );
/**
 ...
 */
function gc_twenty_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'gc-twenty' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'gc-twenty' ) );

	return $args;

}

add_action( 'after_switch_theme', 'gc_twenty_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function gc_twenty_woocommerce_image_dimensions_after_theme_setup() {
	...
	gc_twenty_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'gc_twenty_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function gc_twenty_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function gc_twenty_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'gc_twenty_gallery_image_thumbnail' );
/**
 ...
 */
function gc_twenty_gallery_image_thumbnail( $size ) {
